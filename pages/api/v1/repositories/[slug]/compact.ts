...
export default async function handler(
  ...
  if (process.env.DISABLE_COMPACT_REPO === 'true') {
    return ApiResponse.forbidden(res, 'Compaction is disabled on this server');
  }
  ...
  try {
    ...
    await ShellService.compactRepo(repo.repositoryName);

    return ApiResponse.success(res, `Repository ${repo.repositoryName} has been compacted`);
  } catch (error) {
    if (error instanceof Error && error.message.includes('already running')) {
      return ApiResponse.conflict(res, 'A compaction is already running for this repository');
    }
    return ApiResponse.serverError(res, error);
  }
}
