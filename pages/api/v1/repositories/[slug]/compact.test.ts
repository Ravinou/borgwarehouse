...
describe('Repository compact', () => {
  ...
  it('should return 403 if compaction is disabled on the server', async () => {
    vi.mocked(getSession).mockResolvedValue({ user: { name: 'USER' } });
    process.env.DISABLE_COMPACT_REPO = 'true';
    const { req, res } = createMocks({ method: 'POST', query: { slug: 'abcd1234' } });
    await handler(req, res);
    expect(res._getStatusCode()).toBe(403);
    delete process.env.DISABLE_COMPACT_REPO;
  });
  ...
  it('should return 200 and compact the repository', async () => {
    vi.mocked(getSession).mockResolvedValue({ user: { name: 'USER' } });
    vi.mocked(ConfigService.getRepoList).mockResolvedValue(mockRepoList);
    vi.mocked(ShellService.compactRepo).mockResolvedValue({ stdout: 'ok', stderr: '' });
    const { req, res } = createMocks({ method: 'POST', query: { slug: 'abcd1234' } });
    await handler(req, res);
    expect(ShellService.compactRepo).toHaveBeenCalledWith('abcd1234');
    expect(res._getStatusCode()).toBe(200);
  });

  it('should return 409 if a compaction is already running', async () => {
    vi.mocked(getSession).mockResolvedValue({ user: { name: 'USER' } });
    vi.mocked(ConfigService.getRepoList).mockResolvedValue(mockRepoList);
    vi.mocked(ShellService.compactRepo).mockRejectedValue(
      new Error('A compaction is already running for this repository')
    );
    const { req, res } = createMocks({ method: 'POST', query: { slug: 'abcd1234' } });
    await handler(req, res);
  ...
  it('should return 500 if the shell fails', async () => {
    vi.mocked(getSession).mockResolvedValue({ user: { name: 'USER' } });
    vi.mocked(ConfigService.getRepoList).mockResolvedValue(mockRepoList);
    vi.mocked(ShellService.compactRepo).mockRejectedValue(new Error('borg failed'));
    const { req, res } = createMocks({ method: 'POST', query: { slug: 'abcd1234' } });
    await handler(req, res);
